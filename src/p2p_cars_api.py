...
def _default_demo_data() -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    listings = [
        {
            "id": "lst_00001",
            "seller_id": "user_001",
    ...
    sales = [
        {
            "id": "sal_00001",
            "listing_id": "lst_00099",
            "offer_id": "off_00042",
            "btc_txid": "demo_tx_abc123",
            "status": "settled",
        }
    ]
    return listings, offers, sales
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _json_response(self, code: int, payload: Any, *, head_only: bool = False) -> None:
    ...
    def _route_get(self, path: str, *, head_only: bool = False) -> None:
        if path == "/health":
            return self._json_response(HTTPStatus.OK, {"ok": True}, head_only=head_only)
        if path == "/listings":
            return self._json_response(HTTPStatus.OK, _load_json(LISTINGS_FILE, []), head_only=head_only)
        if path == "/offers":
            return self._json_response(HTTPStatus.OK, _load_json(OFFERS_FILE, []), head_only=head_only)
        if path == "/sales":
            return self._json_response(HTTPStatus.OK, _load_json(SALES_FILE, []), head_only=head_only)
        ...
        return self._json_response(HTTPStatus.NOT_FOUND, {"error": "Not found"}, head_only=head_only)

    def do_GET(self) -> None:  # noqa: N802
        self._route_get(urlparse(self.path).path)

    def do_HEAD(self) -> None:  # noqa: N802
        self._route_get(urlparse(self.path).path, head_only=True)
        ...
        try:
            if path == "/demo/reset":
                ...
                return self._json_response(
                    HTTPStatus.OK,
                    {"ok": True, "listings": len(listings), "offers": len(offers), "sales": len(sales)},
                )
...
def main() -> None:
    ...
    if not LISTINGS_FILE.exists() or not OFFERS_FILE.exists() or not SALES_FILE.exists():
        listings, offers, sales = _default_demo_data()
        _save_json(LISTINGS_FILE, listings)
        _save_json(OFFERS_FILE, offers)
        _save_json(SALES_FILE, sales)
